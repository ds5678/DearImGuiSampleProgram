...
internal sealed class ImGuiController : IDisposable
{
	...
	[MemberNotNull(nameof(_fontTextureView))]
	[MemberNotNull(nameof(_fragmentShader))]
	[MemberNotNull(nameof(_indexBuffer))]
	[MemberNotNull(nameof(_layout))]
	[MemberNotNull(nameof(_mainResourceSet))]
	[MemberNotNull(nameof(_pipeline))]
	[MemberNotNull(nameof(_projMatrixBuffer))]
	[MemberNotNull(nameof(_textureLayout))]
	[MemberNotNull(nameof(_vertexBuffer))]
	[MemberNotNull(nameof(_vertexShader))]
	...
	public unsafe void RecreateFontDeviceTexture(GraphicsDevice gd)
	{
		...
		// Build
		byte* pixels = default;
		int width = default, height = default, bytesPerPixel = default;
	...
	private bool TryMapKey(Key key, out ImGuiKey result)
	{
		...
		result = key switch
		{
			...
			>= Key.Number0 and <= Key.Number9 => KeyToImGuiKeyShortcut(key, Key.Number0, ImGuiKey.D0),
	...
	private void RenderImDrawData(ImDrawData draw_data, GraphicsDevice gd, CommandList cl)
	{
		...
		foreach (ImDrawList cmd_list in draw_data.GetCmdLists())
		{
			cl.UpdateBuffer(
				_vertexBuffer,
